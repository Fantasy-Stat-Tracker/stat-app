class SeasonsController < ApplicationController
  ...
  def index
    ...
    @playoff_games_by_week = @filtered_season.games.where(game_type: "Playoffs").order(:week_number).group_by(&:week_number)
  end

  def show
    season = Season.find(params[:year])
    @member_seasons = season.member_seasons.order("win_percentage DESC")
    @playoff_games_by_week = season.games.where(game_type: "Playoffs").order(:week_number).group_by(&:week_number)
