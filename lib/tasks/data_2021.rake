namespace :data_2021 do
  task :add_games => :environment do
    ...
    players_response = HTTParty.get('http://fantasy.espn.com/apis/v3/games/ffl/seasons/2021/segments/0/leagues/91115?view=mTeam', {headers: {"Cookie": "espn_s2=#{espn_s2}; SWID=#{swid};"}})
    games_response = HTTParty.get('https://fantasy.espn.com/apis/v3/games/ffl/seasons/2021/segments/0/leagues/91115?view=mMatchup', {headers: {"Cookie": "espn_s2=#{espn_s2}; SWID=#{swid};"}})
    year_season = Season.find_or_create_by!(year: 2021, league_id: 1) unless games_response.values.empty?

    #needed to add espn player id's for the season
    players_response["teams"].each do |player|
      p player, "player"
      if player["primaryOwner"] == "{36344C0F-14C1-4EB8-B569-EFC4DDD513A8}"
        member = Member.find_by(espn_id: "{672670FB-77F1-4338-8186-1E308855B6BE}")
      else
    ...
    games_response["schedule"].each do |game|
      ...
      Game.find_or_create_by(game_type: game["matchupPeriodId"] > 14 ? "Playoffs" : "Regular", week_id: week.id, home_score: game["home"]["totalPoints"], away_score: game["away"]["totalPoints"], home_id: home_user.id, away_id: away_user.id)
    end
  end
end